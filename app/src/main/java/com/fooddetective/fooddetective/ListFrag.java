...
public class ListFrag extends ListFragment {
    public StoreListener mStoreListener;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        setListAdapter(new ArrayAdapter<String>(this.getActivity(), android.R.layout.simple_list_item_1,
                getResources().getStringArray(R.array.Store)));
        if (this.getActivity().findViewById(R.id.layout_default)==null){
            mStoreListener.StoreSelected(0);
        }

    }

    public interface StoreListener {
        public void StoreSelected(int index);
    }
    @Override
    public void onAttach(Context context) {
        ...
        try {
            mStoreListener = (StoreListener) context;
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        mStoreListener.StoreSelected(position);
    }
}